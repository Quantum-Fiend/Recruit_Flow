...
import { Search, MapPin, Briefcase, Clock, Filter, Sparkles, ArrowRight, ChevronRight, SlidersHorizontal, Globe } from "lucide-react"
      ...
      <section className="w-full mb-20 space-y-12">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-10">
           <div className="max-w-2xl space-y-6">
              ...
              <h1 className="h-lg text-sapphire">The <br /><span className="text-primary">Network.</span></h1>
              <p className="text-xl text-muted-foreground font-medium">Access world-class engineering roles within the RecruitFlow ecosystem.</p>
           </div>
           
           <div className="flex items-center gap-4">
              <Button variant="outline" className="h-12 px-6 rounded-xl border-border flex items-center gap-2 font-bold text-xs uppercase tracking-widest">
        ...
        <form onSubmit={handleSearch} className="relative group w-full">
          <div className="relative glass-morphism h-20 px-4 flex items-center gap-4 rounded-2xl group-focus-within:border-primary/30 transition-all">
             ...
             />
             <Button type="submit" className="h-14 px-10 rounded-xl sapphire-gradient text-white font-black hover:opacity-90 hidden sm:flex transition-all active:scale-95 shadow-xl shadow-primary/20">
               Initialize Search
             </Button>
          </div>
        </form>
      ...
      <div className="w-full mb-32">
        {loading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {[1, 2, 3, 4, 5, 6].map(i => (
              <Skeleton key={i} className="h-72 w-full rounded-2xl glass-morphism" />
            ))}
          </div>
        ) : jobs.length === 0 ? (
          <div className="text-center py-40 glass-morphism w-full border-dashed rounded-[3rem] flex flex-col items-center">
            ...
            <h3 className="text-3xl font-black mb-4 tracking-tight">System match not found</h3>
            <p className="text-muted-foreground mb-12 max-w-sm font-medium">We could not identify any active positions matching your telemetry profile.</p>
            <Button variant="outline" onClick={() => { setSearch(""); loadJobs(""); }} className="rounded-xl px-12 h-14 font-black border-border">
...
function JobCardItem({ job, index }: { job: any; index: number }) {
  return (
    <motion.div
        ...
        <Card className="glass-morphism h-full p-2 creative-card border-none flex flex-col justify-between">
          <CardHeader className="p-8">
            <div className="flex justify-between items-start mb-8">
              ...
              <div className="badge-premium bg-primary/5 border-primary/10 text-primary">{getJobTypeLabel(job.type)}</div>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-x-5 gap-y-2 text-[10px] font-black uppercase tracking-widest text-muted-foreground">
              ...
              <span className="flex items-center gap-2"><Clock className="w-3.5 h-3.5" /> {formatDate(job.createdAt)}</span>
            </div>
          </CardHeader>
          
          <CardFooter className="p-8 pt-0 mt-auto flex items-center justify-between border-t border-foreground/5 pt-8">
             <div className="flex items-center gap-3">
                <div className="w-8 h-8 rounded-full bg-primary/10 flex items-center justify-center text-[10px] font-black text-primary">RF</div>
